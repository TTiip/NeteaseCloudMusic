...
const sliderBarStyle: any = computed(() => {
  const value = props.modelValue < 0 ? 0 : props.modelValue > 1 ? 1 : props.modelValue
  return props.vertical ? `height:${value * 100}%` : `width:${value * 100}%`
})

// 预加载进度条样式
const preloadStyle: any = computed(() => {
  const value = props.preload < 0 ? 0 : props.preload > 1 ? 1 : props.preload
  return props.vertical ? `height:${value * 100}%` : `width:${value * 100}%`
})
// 预加载进度条样式
const hoverStyle: any = computed(() => {
  const value = state.hoverPosition < 0 ? 0 : state.hoverPosition > 1 ? 1 : state.hoverPosition
...
const mousemoveHandle = (ev: MouseEvent) => {
  ...
  const val = getPosition(ev)
  emits('onMousemove', ev, val)
  state.hoverPosition = val
  if (props.vertical) return
  // 获取dom
  const refSliderEl = (refSlider.value as HTMLButtonElement)
  // 提示宽的一半宽度
  const refTipsWidth = (refTips.value as HTMLButtonElement).clientWidth / 2
  const movePositon = ev.clientX - refSliderEl.getBoundingClientRect().left
  // 如果当前往左的偏移量大于提示框宽度
...
const getPosition = (ev: any) => {
  ...
  if (props.vertical) {
    // 垂直模式下获取高度
    const clientHeight = refSliderEl.clientHeight
