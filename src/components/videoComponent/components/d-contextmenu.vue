...
const menuList = [
  ...
  { label: '版本：' + state.version, key: 'version' }
]
const hotkeyList = [
...
watch(filter, (val) => {
  const dPlayerVideoMain: any = document.querySelector('#dPlayerVideo')
...
const contextmenuShow = (ev: any) => {
  ...
  const refPlayerWrap: any = document.querySelector('#refPlayerWrap')
...
const contextmenuHide = (ev: any) => {
  ...
  if (tagName && hotKeyArr.includes(keycode)) {
    ...
    if (keycode === 'copy') {
      const copyText: any = document.querySelector('.d-player-copyText')
